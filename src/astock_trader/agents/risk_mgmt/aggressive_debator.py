"""激进风控分析师 Agent — 从激进角度评估交易方案的风险。

在风控辩论中代表激进派立场：倾向于接受更高风险以追求更大收益，
关注风险收益比的合理性，主张在可控范围内积极承担风险。
"""
...
def create_aggressive_debator(llm: Any) -> Callable:
    ...
    def aggressive_debator_node(state: dict[str, Any]) -> dict[str, Any]:
        """激进风控分析师节点：从激进角度评估交易风险。"""
        ...
        prompt = (
            f"你是一个激进派风险分析师，正在评估 **{company}** 的交易方案风险。\n\n"
            f"### 完整分析上下文\n{reports}\n\n"
            f"### 风控辩论历史\n{history_text}\n\n"
            f"### 其他风控分析师的论点\n{other_text}\n\n"
            "你代表激进派立场：\n"
            "- 认为风险是收益的代价，合理的风险应当积极承担\n"
            "- 关注上行空间和机会成本\n"
            "- 主张在风险可控的前提下追求最大化收益\n"
            "- 反对过度保守导致错失机会\n\n"
            "请从激进角度给出你的风险评估（用中文），要求：\n"
            "1. 评估交易方案的整体风险水平\n"
            "2. 分析潜在的上行空间\n"
            "3. 回应其他分析师的保守观点\n"
            "4. 给出风险调整后的建议"
        ...
        # 更新风控辩论状态
        new_history = list(risk_history) + [f"[激进派]: {response_text}"]
        new_aggressive_history = list(risk_debate.get("aggressive_history", [])) + [response_text]
